...
namespace OptimizationMethods
{
    ...
    public delegate K MapFunction<K, T>(T item);

    public class TemplateVector<T> : IEquatable<TemplateVector<T>>, IEnumerable<T>, ICloneable where T : new()
    {
        public IEnumerable<ReturnType> Map<ReturnType>(MapFunction<ReturnType, T> fucn)
        {
            foreach (T item in this) yield return fucn(item);
        }

        public static IEnumerable<TemplatePair<T>> Zip(TemplateVector<T> first, TemplateVector<T> second)
        {
            for (int index = 0; index < Math.Min(first.Count, second.Count); ++index)
            {
                yield return new TemplatePair<T>(first[index], second[index]);
            }
        }
        ...
        protected int Filling
        {
            ...
            set
            {
                _filling = Math.Min(Capacity, value);
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        public override string ToString() => $"{{\"vector\":  [{string.Join(", ", Map((v) => v.ToString()))}]}}";
