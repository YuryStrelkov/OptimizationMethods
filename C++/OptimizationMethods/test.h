...
static void simplex_method_test()
{
	...
	sm::simplex sym_0(matrix_f64({ -2, 6, 3, 2, 2, -1 }, 3, 2), { 2, 3 }, { sm::LESS_EQUAL, sm::LESS_EQUAL, sm::LESS_EQUAL }, { 40, 28, 14 });
	sym_0.solve(SIMPLEX_MAX); 
	std::cout << "\n";
	std::cout << " f(x,c) = -2x1 + 3x2;\n arg_min = {7, 0}, f(arg_min) =-14\n\n";

	sm::simplex sym_1(matrix_f64({-2, 6, 3, 2, 2, -1}, 3, 2), { -2, 3 }, { sm::LESS_EQUAL, sm::LESS_EQUAL, sm::LESS_EQUAL }, { 40, 28, 14 });
	...
	std::cout << " | 3x1 + 2x2 >= 28\n";
	std::cout << " | 2x1 -  x2 >= 14\n\n";
	sm::simplex sym_2(matrix_f64({ -2, 6, 3, 2, 2, -1}, 3, 2), {2, 1}, {sm::MORE_EQUAL, sm::MORE_EQUAL, sm::MORE_EQUAL}, {40, 28, 14});
	sym_2.solve(SIMPLEX_MIN);
	std::cout << " f(x,c) =  -2x1 - x2;\n arg_min = {62/5, 54/5}, f(arg_max) = -35 3/5\n";
	std::cout << "\n";
	sm::simplex sym_3(matrix_f64({ -2, 6, 3, 2, 2, -1 }, 3, 2), {-2, -1}, {sm::MORE_EQUAL, sm::MORE_EQUAL, sm::MORE_EQUAL}, {40, 28, 14});
	sym_3.solve(SIMPLEX_MAX);
	std::cout << " f(x,c) =  2x1 + 3x2;\n arg_min = {none, none}, f(arg_max) = none\n";
	std::cout << "\n"; 
	sm::simplex sym_4(matrix_f64({ -2, 6, 3, 2, 2,-1 },3, 2), {2, 3}, {sm::MORE_EQUAL, sm::MORE_EQUAL, sm::MORE_EQUAL}, {40, 28, 14});
	...
	//          2x1 -  x2 = 14
	//
	sm::simplex sym_5(matrix_f64({ -2, 6, 3, 2, 2,-1 }, 3, 2), {2, 3}, {sm::EQUAL, sm::EQUAL, sm::EQUAL}, {40, 28, 14});
