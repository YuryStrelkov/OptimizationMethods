...
1. **C++**
   ...
   - Передавать целевую функцию в виде указателя:  **```typedef F64(*function_1d)(const F64);```**  
     или  
     **```typedef F64(*function_nd)(const numerics::vector_f64&);  //скалярная функция векторного аргумента```**  
     или использовать  
     **```std::function<...(...)>```**;
