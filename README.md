...
4. **Функции внешнего и внутреннего штрафа**
   Основная идея заключается в том, что мы преобразуем исходную задачу:  
   ...
   где слагаемое $$p_{i}\left(\vec{x}\right)$$ является штрафной функцией, которую мы можем предстваить, например, в следующем виде для функции внутреннего штрафа:  
...
Основная идея решения задачи поиска экстремума функции: $$extr\left(f\left(\vec{x}\right)\right),x\in R^{n}$$  
методом внешних штрафов с ограничениями:  
